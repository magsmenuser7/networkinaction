    ...
    <div className="min-h-screen bg-nia-ivory">
      <section className="relative min-h-[60vh] flex items-end pb-20 pt-32 px-6 lg:px-16 overflow-hidden border-b border-nia-border">
        ...
        <div className="max-w-[1400px] mx-auto w-full relative z-10">
          <AnimatedSection>
            ...
            <h1 className="font-playfair text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight max-w-4xl">
              Two leaders.
              <br />
              <span className="text-nia-gold">One mission.</span>
            </h1>
            ...
            {partners.map((partner, i) => (
              <AnimatedSection key={partner.name} delay={i * 0.1}>
                <div className={`grid grid-cols-1 lg:grid-cols-2 gap-0 border border-nia-border rounded-2xl overflow-hidden`}>
                  <div
                    className={`relative h-[400px] lg:h-auto min-h-[500px] bg-cover bg-center ${i % 2 === 1 ? 'lg:order-2' : ''}`}
                      ...
                      <div className="grid grid-cols-3 divide-x divide-white/20">
                        {partner.stats.map((stat) => (
                          <div key={stat.label} className="px-4 first:pl-0 last:pr-0 text-center">
                            <div className="font-playfair text-2xl font-bold text-nia-gold mb-1">{stat.value}</div>
                  ...
                  <div className={`p-10 lg:p-14 flex flex-col justify-center bg-nia-ivory ${i % 2 === 1 ? 'lg:order-1' : ''}`}>
                    <p className="text-xs font-light tracking-[0.4em] uppercase text-nia-gold mb-4">City Partner</p>
                    <h2 className="font-playfair text-4xl md:text-5xl font-bold text-nia-black mb-3 leading-tight">
                      {partner.name}
                    </h2>
                    <p className="text-sm font-light text-nia-muted tracking-widest uppercase mb-4">
                      {partner.tagline}
                    </p>
                    <p className="text-xs font-light text-nia-gold tracking-wide mb-6">{partner.role}</p>
                    <p className="text-base font-light text-nia-gray leading-relaxed-custom mb-8">{partner.desc}</p>
                    <div className="flex flex-wrap gap-2 mb-10">
                      {partner.ventures.map((v) => (
                        <span key={v} className="text-xs px-3 py-1.5 border border-nia-gold/40 text-nia-gold font-light">
                          {v}
                        </span>
                      ))}
                    </div>
      ...
      <section className="py-24 px-6 lg:px-16 bg-nia-darker border-y border-nia-border">
        <div className="max-w-[1400px] mx-auto">
          <AnimatedSection className="mb-16">
            <p className="text-xs font-light tracking-[0.4em] uppercase text-nia-gold mb-4">Powered By</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-nia-border">
            {poweredBy.map((item, index) => (
              <AnimatedSection
                key={index}
                delay={index * 0.08}
                className="bg-nia-darker border border-nia-border rounded-xl p-8 hover:bg-nia-ivory hover:border-nia-gold/40 hover:shadow-editorial transition-all duration-300"
              >
                <h3 className="font-playfair text-xl font-bold text-nia-gold mb-4">{item.name}</h3>
      ...
      <section className="py-24 px-6 lg:px-16 bg-nia-black">
        <div className="max-w-[800px] mx-auto text-center">
          <AnimatedSection>
            <p className="text-xs font-light tracking-[0.5em] uppercase text-nia-gold mb-6">Join the Room</p>
